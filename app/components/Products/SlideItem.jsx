...
const SlideItem = ({obj, index, refFunction, touchStart, touchEnd, touchMove, mouseOver, canClick, cameFromLeft, left}) => {
  return (
    // image draggable = false onDragStart={(e) => e.preventDefault()}
    <div ref={refFunction} className={`lg:w-[30vw] w-[85vw] flex-shrink-0 border-r border-black ${index == 1 && 'bg-white'} ${index == 3 && 'bg-white'}`} 
        ...
        <h3 className=' ml-4 text-xl tracking-wide font-[Montserrat] font-medium'>BeautyWorks Nourishing Mask</h3>
            <div className='h-[2px] ml-6 bg-black w-[85%]'></div>
            <h3 className='ml-6 font-medium text-xl mb-3'>£15.00</h3>
            <p className='text-sm font-medium  pl-6 pr-8 max-h-[15vh] overflow-y-scroll font-[Montserrat] text-zinc-800'>
