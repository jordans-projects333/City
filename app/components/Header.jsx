...
const Header = ({hamburger, setHamburger, productSnap, serviceSnap, homePage, homeSwipeLeft, homeSwipeRight, serviceClickable, onService}) => {
    ...
    return (
        <header ref={header} className={`fixed z-50 flex duration-500 h-[6vh] w-full nav__wrapper ${onService && '!bg-white'}`}>
            ...
            <Nav hamburger={hamburger} setHamburger={setHamburger} header={header} productSnap={productSnap} serviceSnap={serviceSnap} homePage={homePage} homeSwipeRight={homeSwipeRight} homeSwipeLeft={homeSwipeLeft} serviceClickable={serviceClickable}/>
        </header>
    )
}
